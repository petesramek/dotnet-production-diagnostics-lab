...
/// <summary>
/// Maps endpoints for unbounded retries and controlled retry scenarios.
/// </summary>
public static class UnboundedRetriesEndpoints {
    /// <summary>
    /// Adds retry behavior diagnostics endpoints to the endpoint route builder.
    /// </summary>
    public static IEndpointRouteBuilder MapUnboundedRetriesEndpoints(this IEndpointRouteBuilder endpoints) {
        var group = endpoints.MapGroup("/07-unbounded-retries");
            ...
            for (var attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                try {
                    ...
                    logger.LogWarning(exception,
                        "Inventory attempt {Attempt} failed for SKU {Sku}; retrying immediately",
                        attempt,
                        sku);
                }
            }
            ...
            for (var attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                try {
                    logger.LogInformation(
                        "Inventory attempt {Attempt} of {MaxAttempts} for SKU {Sku}",
                        attempt,
                        maxAttempts,
                        sku);
                    ...
                    logger.LogWarning(exception,
                        "Inventory attempt {Attempt} failed for SKU {Sku}; waiting {DelayMs} ms before retry",
                        attempt,
                        sku,
                    ...
                    logger.LogError(exception,
                        "Inventory provider unavailable for SKU {Sku} after {Attempts} controlled attempts",
                        sku,
                        attempts);
                }
            }
